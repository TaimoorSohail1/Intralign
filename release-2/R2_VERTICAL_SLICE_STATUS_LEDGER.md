...
| **3 · Reanalysis Engine + Freeze/Unlock** | **IMPLEMENTED · COMPLETION GATES OPEN (2026-08-12).** Fast/Deep pass metadata, project-level batching, STALE/reanalyzing/fresh state, explicit reanalysis, one transient retry, last-good preservation, pending Undo, append-only Deep supersession, causal read-moved notification, first-run outcome confirmation, presentation-only freeze, and latched two-act unlock are implemented. Migration `20260812100000_r2_slice_3_reanalysis.sql` is applied locally. | **IMPLEMENTED; MANUAL MATCHED-STATE REVIEW BLOCKED.** The first-analysis UI now follows the signed analysis animation, graph, and outcome-confirmation states, and Overview exposes STALE, Reanalyze now, Undo, read-moved, and freeze/unlock states. Automated component coverage passes, but the in-app browser rejected the local page reload under its URL security policy, so no new combined prototype/implementation comparison is claimed. | **PARTIAL · AUTOMATED REGRESSION PASS, LIVE BROWSER PASS OPEN.** API 303/303 and web 131/131 pass; production build and guardrails pass. The live first-read, batching, Undo, failure/retry, persistence, responsive, zoom, reduced-motion, and shared spoken-screen-reader journeys remain to be executed in the approved browser. | **PASS for implemented contracts.** Full API suite: 303 passed. Full web suite: 23 files/131 passed. Ruff, ESLint, TypeScript/Next production build, and R2 guardrails pass (4 infrastructure + 9 active selectors). | **NOT VERIFIED.** Exact matched-state prototype comparison remains a blocking completion gate; automated tests and build success are not substituted for it. | **Active coverage added and passing:** batching and merge semantics, Fast harness budget, Deep supersession event, retry-once policy, STALE projection, explicit run, outcome actions, pending Undo, first-run count/latch, read-moved notification, analysis animation/outcome UI, and Overview stale/Undo UI. Shared spoken screen-reader test remains owner-deferred until Slices 1–3 are otherwise ready. | Run the live matched-state browser audit and manual regression when the local URL is permitted, fix any visible P0/P1/P2 drift, then run the combined Slices 1–3 spoken screen-reader gate. Keep Slices 4–10 blocked. |
