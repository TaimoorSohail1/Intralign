...
import { listInvitations } from "@/lib/server/oslo-api";
import { readSession } from "@/lib/server/session";
import { logout } from "@/app/logout-action";

import { inviteMember, resendMemberInvitation, revokeMemberInvitation } from "./actions";

interface InvitationsPageProps { searchParams: Promise<{ sent?: string; updated?: string }> }
...
export default async function InvitationsPage({ searchParams }: InvitationsPageProps) {
  ...
  const { sent, updated } = await searchParams;
    ...
    <main className="admin-shell">
      ...
      <section className="admin-content">
        ...
        <form action={inviteMember} className="invite-form">
          <div className="field"><label htmlFor="invite-email">Email address</label><input id="invite-email" name="email" required type="email" /></div>
