...
**Application branch:** `feature/r2-complete-application`
...
| Branch name | Yes | `feature/r2-complete-application` |
...
git switch feature/r2-complete-application
cd apps/intralign
corepack enable
...
Create `apps/intralign/.env` only if the developer wants to override the safe local Docker defaults:
...
Run the checks from `apps/intralign/`:
