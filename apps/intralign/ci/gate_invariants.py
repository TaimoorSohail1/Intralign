...
a. **Forbidden-token scan** — any occurrence of ``GovernanceDecision`` or
   ...
   files under ``backend`` and ``shared`` fails, EXCEPT lines listed in
   the explicit allowlist file ``ci/invariant_allowlist.txt`` (existing
...
b. **No Authority module** — a directory named ``authority`` anywhere under
   ``backend`` fails (R1: specified-but-inactive; never built).
c. **Migration linter** — ``.sql`` files under ``supabase/migrations/`` must
   ...
   **Reviewed-allowlist exemption** — a canonical-table ``ALTER`` whose statement
   ...
   allowlist (``ci/invariant_allowlist.txt``) is exempted; this exists ONLY
...
parent, i.e. the application root) and exits 0 (pass) / 1 (fail), printing every violation.
"""
...
# Roots scanned for forbidden tokens, relative to the application root.
SCAN_ROOTS: tuple[str, ...] = ("backend", "shared")

# (b) Forbidden module name under backend.
...
def check_authority_dir(code_root: Path) -> list[str]:
    """(b) Flag any directory named ``authority`` under backend."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Application root. Default: parent of ci/.",
    )
