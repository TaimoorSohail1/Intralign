# apps/intralign/ci — App CI gate logic (DTM-0001)
...
Run the Phase 0 gate locally from `apps/intralign/`:
...
Run the gate-script tests locally (from `apps/intralign/`):
...
Run a gate by hand (from `apps/intralign/`):
...
4. **Gate 4 (invariants):** any one of:
   ...
   - `mkdir apps/intralign/backend/authority` with a `.gitkeep` (forbidden module);
   - add `apps/intralign/supabase/migrations/9999_bad.sql` containing
