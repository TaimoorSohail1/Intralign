...
def _validate_surface_registry(
    ...
    registry_path = application_root / "ci" / "r2_surface_contracts.json"
...
def evaluate_repository(application_root: Path) -> GateReport:
    registry_path = application_root / "ci" / "r2_guardrails.json"
    registry = _load_registry(registry_path)
    contract_path = application_root / registry["contract_path"]
    ...
    prototype_path = application_root / "ci" / "contracts" / "oslo-prototype-r2.html"
    ...
    for guard_id, registration in registered.items():
        ...
        if status == "pending":
            ...
            for selector in selectors:
                ...
                if not (application_root / test_path).is_file():
            ...
            for client_test in client_tests:
                if not isinstance(client_test, str) or not client_test.strip():
                    ...
                    continue
                if not (application_root / client_test).is_file():
                    errors.append(
...
def run_active_tests(report: GateReport, application_root: Path) -> int:
    ...
    if report.active_client_test_files:
        ...
        return subprocess.run(command, cwd=application_root, check=False).returncode
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = evaluate_repository(application_root)
    print(
        "[R2 guardrails] "
    ...
    if report.errors:
        ...
        return 1
    if args.run_active:
        return run_active_tests(report, application_root)
    return 0
