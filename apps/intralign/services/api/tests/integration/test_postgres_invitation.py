...
def isolated_invitation_workspace():
    ...
    def reset_workspace() -> None:
        with engine.begin() as connection:
            ...
            connection.execute(
                text(
                    "delete from public.workspace_subscriptions where workspace_id = :workspace_id"
                ),
                {"workspace_id": WORKSPACE_ID},
            )
            connection.execute(
                text(
                    "delete from public.workspace_limit_events where workspace_id = :workspace_id"
                ),
                {"workspace_id": WORKSPACE_ID},
            )
...
def test_concurrent_activation_submissions_are_idempotent() -> None:
    ...
    # Exercise the single-connection pool used in production. Activation must
    # not deadlock while holding the advisory-lock connection.
    ...
    try:
        ...
        if delegate_user_id is not None:
            with engine.begin() as connection:
                ...
                connection.execute(
                    text(
                        "delete from public.workspace_notification_reads where user_id = :user_id"
                    ),
                    {"user_id": delegate_user_id},
                )
                connection.execute(
                    text(
                        "delete from public.workspace_member_preferences where user_id = :user_id"
                    ),
                    {"user_id": delegate_user_id},
                )
                connection.execute(
