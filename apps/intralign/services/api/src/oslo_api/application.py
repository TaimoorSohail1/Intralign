...
class CollaboratorSeatLimitReached(Exception):
    """Raised when another owner would exceed the workspace seat cap."""

    def __init__(self, policy: PlanPolicy | None = None) -> None:
        policy = policy or get_plan_policy("free")
        self.plan = policy.code.value
        self.collaborator_seat_limit = policy.collaborator_seat_limit
        self.remedies = ("compare_plans",)
        super().__init__("Workspace owner seat limit reached")
...
class DatabaseSliceOneApplication:
    ...
    def _workspace_access_role(
        ...
        owner_role = SqlMembershipReader(connection).role_for(workspace_id, actor_user_id)
    ...
    def get_workspace_summary(self, *, actor_user_id: UUID, workspace_id: UUID) -> WorkspaceSummary:
        with self._engine.connect() as connection:
        ...
        return WorkspaceSummary(
            ...
            can_create_project=(
                ...
                and sum(row["archived_at"] is None for row in rows) < policy.active_project_limit
            ),
        )
    ...
    def archive_project(self, *, actor_user_id: UUID, workspace_id: UUID, project_id: UUID) -> None:
    ...
    def restore_project(self, *, actor_user_id: UUID, workspace_id: UUID, project_id: UUID) -> None:
        self._set_project_archived(
            actor_user_id=actor_user_id,
            workspace_id=workspace_id,
            project_id=project_id,
    ...
    def _activate_invitation_once(
        ...
        *,
        ...
        if self._identity is None:
            raise RuntimeError("Identity provider is not configured")
        invitation = self._resolve_pending_invitation(
            token,
            allow_accepted=True,
            connection=connection,
        )
    ...
    def _accept_invitation_for_existing_user_once(
        ...
        *,
        ...
        return ActivationResult(
            ...
            refresh_token=session.refresh_token,
            expires_in=session.expires_in,
            welcome_required=(accepted_role == MembershipRole.OWNER.value and not access_existed),
        )
    ...
    def _resume_accepted_invitation(
        ...
        *,
        ...
        if session.user_id != invitation["accepted_by"]:
            raise InvalidInvitation
        accepted_workspace_id = invitation["accepted_workspace_id"] or invitation["workspace_id"]
        membership = (
            connection.execute(
                text(
                    """
    ...
    def _resolve_pending_invitation(
        ...
        *,
        ...
        if connection is None:
            with self._engine.connect() as checked_out_connection:
                return self._resolve_pending_invitation(
                    token,
