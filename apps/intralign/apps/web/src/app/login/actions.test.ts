...
const { acceptExistingInvitation, getSessionContext, redirect, writeSessionCookies } = vi.hoisted(() => ({
...
describe("signIn", () => {
  ...
  it("returns an invited account to a recoverable login state when the API is unavailable", async () => {
    acceptExistingInvitation.mockRejectedValueOnce(new Error("API unavailable"));
    const formData = new FormData();
    formData.set("invitation_token", "invite-token");
    formData.set("email", "person@example.com");
    ...
    await expect(signIn(formData)).rejects.toThrow(
      "REDIRECT:/login?invite=invite-token&error=service_unavailable",
    );
