...
import type { ActivationActionState } from "@/app/activate/actions";
...
interface ActivationFormProps {
  ...
  action?: (
    previousState: ActivationActionState,
    formData: FormData,
...
function fallbackAction(): Promise<ActivationActionState> {
...
export function ActivationForm({ email, token, action }: ActivationFormProps) {
  const [state, formAction, pending] = useActionState(action ?? fallbackAction, initialState);

  return (
    <form action={formAction} className="activation-card">
      ...
      {state.error ? (
        <p aria-live="polite" className="login-error" role="alert">{state.error}</p>
      ) : null}
