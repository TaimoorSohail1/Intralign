...
## U. Ratified update — shell declutter & navigation (DL-088, 2026-07-02)
...
Ratified by **DL-090** (presentation-only). The **Attention map (MRI Workspace)** is a **left-rail center-pane surface** (consistent with the canonical MRI Workspace primary nav), reachable alongside Overview / Findings / History — **not embedded in the Overview**. Correspondingly the Overview no longer hosts the heatmap, the reliability component breakdown, or a recommendations list (see `PROJECT_OVERVIEW_SCREEN_SPEC` §R). Visual reference of record: `product-design/oslo_r1_experience_mockup_v4.html` (baseline `07-attention-map`).
