...
**Resolution (amended by DL-089, 2026-07-02 — Option B):** Chat is the **primary** clarification interface, **and** a clarification may **also be answered in-context from the Finding Panel** (the prototype's inline affordance). Both surfaces route through the **same** information-capture path — the answer is a project-information change that feeds the next reanalysis (Q9); it is **still not a modeled object** (§122) and **changes no assessment by itself**. A finding with a pending clarification shows an inline prompt/answer affordance; Chat remains the conversational home for clarification. *(Prior R1 resolution centralized clarification in Chat only; DL-089 permits the Finding-Panel surface as a second entry point without changing the object-free semantics.)*
