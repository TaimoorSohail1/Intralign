...
| PF-03 | Project & Workspace Lifecycle | Persistent Project within Workspace/Account; container for all objects | F | Durable project state | Alpha | Critical | PLAT-01, PLAT-06 | §18 | — | Proj, WS, Usr | Overview, Settings | N | One **active** project on Free tier (see MON-01); **Create Project stays enabled at the cap — attempt gated (API 422) → UP-3 upgrade-or-archive (CHG-065)** |
...
| MON-02 | Daily Fix Allowance & Reset | Daily suggested-fix allowance that resets automatically | C | Habit + re-engagement | Alpha | High | REC-04 | §13, §16 C13 | C13 | Fix, Usr | IssuePanel | N | Drives re-engagement and upgrade; **limit-reached interaction rule — affordance stays enabled → 429 → UP-1 (Seam Audit 001 S1)** |
| MON-03 | Usage Limits | Limit recommendation applications and chat usage on Free tier | C | Clear upgrade reason | Alpha | Medium | CHAT-01, REC-02 | §13 | C13 | WS | Settings | N | Limited chat / limited rec applications; **chat/deep caps → 429 → UP-2/UP-5/UP-6 via the shared limit-reached rule (Seam Audit 001 S2/S3)** |
...
3. **Templates — RESOLVED (DL-056, 2026-06-10).** ~~referenced but undefined~~ → `RELEASE_1_TEMPLATE_INTAKE_SPECIFICATION_V1` defines a **fully-worked-fictitious-sample** template instantiated as a **populated sample project**, a **curated catalog of five** (Office Relocation / "Pulse" launch / "Brew & Co" campaign / "DevNorth 2026" event / EU expansion OKR), owner-curated only, one editable evidence-class Artifact (`source="template"`) on the standard Fast Pass path (MRI in ~60s); `project_type` pre-filled, non-gating; project flagged a sample; no system generation. **In scope for Release 1.**
...
11. **Acceptance Criteria coverage — RECONCILED (KIA2-4, 2026-06-05).** ~~partial~~ → AC bases now exist and are mapped: **cognitive** capabilities → Wave QA contracts (`QA-WB/WC/WU/WE/WS/WI`) + `RELEASE_1_BUILD_TEST_OBSERVE_TRACEABILITY_MATRIX.md` (Test-basis column); **Telemetry/Security/Platform** → the Observability & Economics Platform spec + Deployment Governance + commodity normal-engineering (DL-043 J) + this session's QA (DL-048 cost gate, Seam-Audit limit-reached, Internal-bypass). **Only open AC = CAF/Confidence numeric formula (TBD-by-design, Open-TBD F1).** Intentional commodity non-contract is **not** an AC defect (KIA2-5). Full map: `03_architecture/reviews/RELEASE_1_ACCEPTANCE_CRITERIA_COVERAGE_RECONCILIATION_001.md`.
