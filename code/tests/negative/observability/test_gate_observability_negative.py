...
- a tampered A6 vocabulary (renamed, extra, or non-literal EVENT_NAMES) across
  ...
  WB_EVAL (DTM-0011) / WC_ADVISE (DTM-0014) / COST — plus a drifted or short
  8-way union.
"""
...
def test_missing_event_names_assignment_fails(tmp_path) -> None:
    ...
    violations = check_event_vocabulary(root)
    # all eight contract tuples (WA00R/WA001/WA002/WS/WB_INFER/WB_EVAL/WC_ADVISE/COST) AND the union.
...
def test_union_dropping_ws_and_cost_legs_fails(tmp_path) -> None:
    """DTM-0014 — a union that omits the WS/WB_INFER/WB_EVAL/WC_ADVISE/COST legs (an old form) fails."""
    tampered = GOOD_EVENTS_PY.replace(
        "EVENT_NAMES: tuple[str, ...] = (\n"
        "    EVENT_NAMES_WA00R\n"
        "    + EVENT_NAMES_WA001\n"
