...
| # | Criterion | Proof |
|---|---|---|
| **C-DL109-1** | **GROUNDED FACTS ARE CLAIM-LEVEL AND COMPUTED** (DL-111 foundation bar, erratum) — the **hero is the GROUNDED count ALONE**: attested claims (*Confirmed by you*, `evidence_id` present), **never** attested + inferred. The grounded segment carries **only its label** (the number is the hero); the *From OSLO* segment is the **inferred** state and carries its count. Both read the nullable `evidence_id` through `_ciEvidenceId()`, **computed never typed**. **No denominator.** | ✅ `_assertPgxBarIsComputedFromRealCounts()` (hero == grounded, RED on grounded+inferred) · `_assertProgressCountsAreComputed()` · `_assertEveryPayoffCountIsComputed()` |
...
> bar**: a computed **grounded-facts hero (ATTESTED claims only)**, a **proportional solid bar** showing **two
...
| **C-D194-2** | ⛔⛔⛔ **THE BAR'S SEGMENTS SPEAK THE RATIFIED EPISTEMIC CLASSES.** *Rendered (erratum):* the grounded segment names **Confirmed by you** (label only — its count is the **17 grounded-facts hero**) and the inferred, hatched segment names **From OSLO 12**. **Exact ratified strings** (D011/D069). | ✅ `_assertEpistemicClassNamesAreSingleSourced()` clause (e) — the bar segments must NAME both classes, read from the registry |
...
| **C-D194-5** | ⛔⛔ **THE SOLID BAR AND THE LOAD-BEARING LINE COUNT DIFFERENT POPULATIONS.** (DL-111 + erratum.) The bar's inferred segment counts inferred **claims** (12); the leans line counts inferred items of **every** type (20), so **20 legitimately exceeds 12** — a superset, **never `+`-joined** into the bar. **No denominator, no percentage, no "of", no ratio glyph** on either. | ✅ `_assertPgxBarStructure()` (load-bearing never inside the bar / `+`-joined) · `_assertProgressRowsStayDistinct()` · NC-D194-11 (the merge, built) · NC-D194-13 (the counts read as a ratio) |
