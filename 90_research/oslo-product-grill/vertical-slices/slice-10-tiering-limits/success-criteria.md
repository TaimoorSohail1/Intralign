...
## DL-109 — Surface the provenance · reject the debt frame

| # | Criterion | Proof |
|---|---|---|
| **C-DL109-1** | **GROUNDED FACTS ARE CLAIM-LEVEL AND COMPUTED** (DL-111 foundation bar, erratum; **WI-R6 fraction hero**) — the **hero's PRIMARY number is the GROUNDED count ALONE**: attested claims (*Confirmed by you*, `evidence_id` present), **never** attested + inferred. It is shown as a **fraction — `17 of 28`** (grounded of **total claims** = grounded + inferred), captioned *"grounded in your evidence / the rest is OSLO's inference"*; the **28** denominator is a **composition** (what rests on you vs OSLO) — **computed, not a burndown target** (inference is not driven to zero). The grounded segment carries **only its label** (the numerator is the hero); the *From OSLO* segment is the **inferred** state and carries its count. All read the nullable `evidence_id` through `_ciEvidenceId()`, **computed never typed**. | ✅ `_assertPgxBarIsComputedFromRealCounts()` (hero numerator == grounded, RED on grounded+inferred; **denominator == grounded + inferred**) · `_assertProgressCountsAreComputed()` · `_assertEveryPayoffCountIsComputed()` |
| **C-DL109-2** | ⭐ **LOAD-BEARING INFERENCES ARE COMPUTED — D181a, three clauses.** *The read would change were it false; operationally **the read points at it**.* **(a)** a critical issue cites it · **(b)** the limiting dimension's assessment rests on it · **(c)** ⭐ a strong-reading artifact's confidence rests on it. **Boot: 12** *(a 3 · b 8 · c 4 — **Scope's four are IN, via clause (c)**)*. **State proof, every clause.** | ✅ `_assertLoadBearingIsComputed()` · NCs `clauseC_isDeleted_bites` · `clauseA_isDeleted_bites` · `clauseB_isDeleted_bites` · `theLooseReadingReturns_bites` · `theNumberIsTyped_bites` · `theNumberIsHidden_bites` |
| **C-DL109-3** | ⛔⛔ **NO DEBT FRAME, ANYWHERE.** No surface says *owed · debt · liability · pay down · burndown · drive to zero*. Graded on **four doors**: cascade (a `.debt-*` class) · copy · **render path** · **registry label**. | ✅ `_assertNoDebtVocabulary()` · NCs `theRejectedFrame_understandingDebtReturns` · `theFrame_inThePainter` · `theFrame_inTheRegistry` · `theFrame_aDebtClassInTheCascade` — **all bite** |
| **C-DL109-4** | ⛔⛔⛔ **A RISING INFERENCE COUNT IS NEVER STYLED OR WORDED AS A REGRESSION.** More inferences **and** a higher band is **correct** (D177). | ✅ `_assertRisingInferenceIsNotARegression()` (state proof) · **must-not-fire `mustNotFire_moreInferencesAndAHigherBand` stays green** across six guards |
| **C-DL109-5** | **The Inference map uses NO percentage fill and NO severity colour.** The pips are a **count**: per row, the marks on screen **equal the items in the model**. | ✅ `_assertInferenceMapIsNeutral()` (cascade + DOM + render path + the pip-equals-item proof) · NCs `theMapBecomesABar` · `inferenceIsPaintedAsSeverity` · `thePipsStopCounting` — all bite |
| **C-DL109-6** | ⭐ **The map FLAGS the confident-but-inferred artifact BY NAME** (CONF-06 at the artifact level), and it is **computed**: construct the condition → it appears and names it; **ground the artifact → it goes.** | ✅ `_assertInferenceMapNamesTheArtifact()` · NCs `theFindingIsHardCoded` · `theFindingIsSilent` — both bite. Live: **Scope**. |
| **C-DL109-7** | **The Readout's "Key assumptions" reads the ASSUMPTION REGISTER, not the clarification proxy.** Proven **both ways**: a new assumption **appears**; a new clarification **does not**. | ✅ `_assertKeyAssumptionsReadTheRegister()` · NC `theProxyReturns_bites` |
| **C-DL109-8** | **"What I'd need to be sure" is in the Readout**, as a list of asks, **audience-independent**. | ✅ `_assertReadIdenticalAcrossAudience()` now grades §5 · `_assertReadoutRunsNoAnalysis()` exercises it |
| **C-DL109-9** | ⛔ **INFERENCE CHAINS ARE NOT SMUGGLED IN.** No `derived_from`, no field pointing at another ContextItem, no painter that computes a depth. | ✅ `_assertNoInferenceChains()` · NC `theChainIsSmuggledIn_bites` |
| **C-DL109-10** | **The provenance counts have ONE home** (Progress). The Inference map **points**; it does not repeat. | ✅ `_assertProvenanceCountsHaveOneHome()` · `_assertNoCountIsRenderedTwice()` (vacuity floor raised to **8**) |
| **C-DL109-11** | **97 boot assertions green · 0 console errors · Free × Basic × notes-OFF × notes-ON.** NC suites: **D109 32/32 · D180 19/19 · D179 28/28 · D177 13/13 · D176 17/17 · D170 26/26 · D164 47/47.** | ✅ jsdom |

## D181 — "Load-bearing" = the read points at it · Age the clock, not the past

| # | Criterion | Proof |
|---|---|---|
| **C-D181-1** | ⭐⭐ **THE NUMBER INCLUDES SCOPE.** Clause (c) — *a strong-reading artifact's confidence rests on it* — puts **CI-20 · CI-21 · CI-22 · CI-23** in the count. **The strict reading would have excluded them, and missed FALSE CONFIDENCE entirely.** *Scope reads fine **because of** four things OSLO made up.* | ✅ `_assertLoadBearingIsComputed()` grades the flagged artifact's inferences **by identity** — live **and** constructed |
| **C-D181-2** | ⛔ **CLAUSE (c) IS NON-NEGOTIABLE — delete it and the guard goes RED.** | ✅ **NC `clauseC_isDeleted_bites`** (mandated by D181a) — and `clauseA` · `clauseB` · `theLooseReadingReturns` bite too |
| **C-D181-3** | **AN INFERENCE NOTHING POINTS AT IS NOT COUNTED.** Push an item bearing on the limiter that nothing cites → **the number must not move.** *(This is the over-count the loose reading shipped: 11, incl. CI-45 · CI-56 · CI-57.)* | ✅ state proof inside `_assertLoadBearingIsComputed()` |
| **C-D181-4** | ⛔⛔ **GROUNDING THE FLAGGED ARTIFACT RETIRES CLAUSE (c) — AND THAT IS CORRECT, NOT A GUARD FAILURE.** Deep pass → **20**; the user grounds Scope → **12**, clause (c) → **0**, the flag goes. **Every DL-109 guard stays GREEN.** *(A previous guard in this file called exactly this the user's failure — D166 §3.)* | ✅ **must-not-fire `mustNotFire_groundingTheFlaggedArtifactRetiresClauseC`** · the guard's own constructed proof is **order-independent** |
| **C-D181-5** | ⭐ **ADVANCING A WEEK AGES THE ASSUMPTIONS.** `simNextWeek()` → *"Unvalidated for 2 minutes"* becomes *"Unvalidated for **3 weeks** · 1 issue depends on it."* **Computed from the clock (`_ciNow()`), never typed** — and the dependent-issue count is computed too. | ✅ `_assertSimNextWeekAgesTheAssumptions()` (advance → the age must GROW, in weeks, **on screen**) · NC `ageingIsTyped_bites` |
| **C-D181-6** | ⛔ **THE DEMO PROJECT IS NOT BACK-DATED. D100 HOLDS AT WEEK 0** — the project genuinely is minutes old, and says so (`now − 2m`). | ✅ `_assertD100FirstRunStateHoldsAtWeek0()` · **NC `thePastIsBackDated_bites`** |
| **C-D181-7** | **GROUNDING VELOCITY MOVES WITH THE CLOCK.** The window is the current demo week: three weeks on with nothing done it reads **0 · 0** — *understanding is stalling*, honestly stated. **Never a target, no denominator.** | ✅ inside `_assertSimNextWeekAgesTheAssumptions()` |
| **C-D181-8** | **NO REGRESSION.** 97 boot assertions × Free × Basic × notes-OFF × notes-ON · **0 console errors** · NC D109 **32/32** · D180 **19/19** · every must-not-fire control **green** · AA both themes. | ✅ jsdom |
| **C-DL109-12** | **AA in both themes** on every new element. **One real AA defect found and fixed:** the inferred pip's edge was **1.58 : 1** (below the 3:1 non-text minimum for a mark that carries information). | ✅ computed from the live tokens |

---

# D182 / D183 — SUCCESS CRITERIA (2026-07-13)

| # | Criterion | Verified |
|---|---|---|
| **C-D182-1** | ⛔⛔⛔ **NO PROBE CAN RAISE A PROMPT AT THE USER.** Run the **entire** guard suite plus **all eight** other negative-control suites: **prompts that reached the user = 0.** Prompts **computed** under the fence = **218** (so the proof is not vacuous). | ✅ `_assertNoProbeCanRaiseAPrompt()` · behavioural harness |
| **C-D182-2** | ⛔⛔ **NO PROBE LEAVES RESIDUE.** localStorage · the append-only History · Trend · the memo register · the chat thread · the deferral queue · the open prompt surfaces — **byte-for-byte, across every guard and every NC suite: 0 dirty.** | ✅ `_assertProbeLeavesNoResidue()` |
| **C-D182-3** | **THE DEFERRAL QUEUE IS NEVER ARMED BY A PROBE** — the delayed bomb that lands on the user with no attempt. After the full suite: **queue length 0.** | ✅ `_S10.d182FenceLifted` (checked from **outside** every probe) |
| **C-D182-4** | **A PROMPT MAY FIRE ONLY FROM A REAL USER ATTEMPT** (D138/D170). Free + a Basic export format, fired **outside any fence** → prompt shown, **limit named ("Export formats")**, **2 resolutions**. | ✅ **must-not-fire** control |
| **C-D182-5** | **EVERY PROBE IS FENCED BY CONSTRUCTION**, not by remembering to. **Exactly one named exemption** — the suite that grades the fence. | ✅ clause (d) |
| **C-D182-6** | **THE NEGATIVE CONTROL BITES:** make a probe fire a prompt (remove the fence) → **the fence guard goes RED.** | ✅ **NC `aProbeThatFiresAPromptAtTheUser_bites`** |
| **C-D183-1** | **OSLO says "I" ONLY in chat.** Zero first person across every OSLO-voiced panel. **The PM still says "I" in the readout** (D152) and **the chat still says "I"** (D165). | ✅ `_assertOsloSaysIOnlyInChat()` + two must-not-fire controls |
| **C-D183-2** | ⭐ **"OUTCOME CONFIDENCE" EVERYWHERE · ZERO `/100` · the 0–100 index DELETED** from the DOM, the render paths, the payoff snapshot **and the cascade**. Pill reads: **"Outcome Confidence · Moderate · largely grounded".** | ✅ `_assertNoZeroToHundredIndexAnywhere()` |
| **C-D183-3** | **CONFIDENCE AND GROUNDING SHARE NO WORD.** Grounding: *barely / thinly / partly / largely / well grounded*. Band: *Very Low / Low / Moderate / High / Very High*. **Intersection: empty**, on word boundaries. The grounding word is **computed from `CONTEXT_ITEMS`** (state proof: ground every claim and it moves). | ✅ `_assertGroundingSharesNoWordWithTheBand()` |
| **C-D183-4** | **"inferences", not "things".** The Progress panel's load-bearing line reads: *"Your read leans on **20** inferences — the inferred claims above plus inferred assumptions, relationships and metrics"* (DL-111 + erratum). | ✅ `_assertInferencesAreCalledInferences()` |
| **C-D183-5** | **DOCUMENTS, not artifacts/sections** — every user-facing surface, **including tooltips and aria-labels**. The canonical entity remains `Artifact` (DL-095 split). | ✅ `_assertPlanDocumentsAreCalledDocuments()` (rendered DOM + attributes) |
| **C-D183-6** | **THE TREND LINE HAS NO CAUSE TEXT** — sparkline + direction word only (*"↗ Strengthened"*). The cause is still **computed** and still **shown**, in "What changed", and **nowhere else**. Causes have one home, like counts (D179e). | ✅ `_assertTrendLineCarriesNoCause()` |
| **C-D183-7** | **THE OVERVIEW ORDER FLIPS ON FIRST VALUE, COMPUTED.** First run → **Start here → Progress**. After first value → **Progress → Start here**. Never a static order. | ✅ `_assertOverviewOrderFlipsOnFirstValue()` (moves the state, re-reads the DOM) |
| **C-D183-8** | **NO REGRESSION.** **106 boot assertions × Free × Basic × notes-OFF/ON × first-value NO/YES (8 configurations) · 0 console errors** · **9 NC suites, 204 controls, every one bites** · every must-not-fire control green · **AA both themes** (`--text` 16.9/16.6 · `--muted` 9.9/8.0 · `--subtle` 6.0/5.2). Slices 1–9 guards all green; body children 31, panes 7, unchanged. | ✅ jsdom |

| **C-D184-1** | ⛔⛔⛔ **THE FIX IS ON SCREEN, ABOVE ITS BUTTON.** Issue panel leads with *"Confirm the venue's 500-person Wi-Fi capacity before locking the in-person-only format."*; the button reads **"Apply this fix"** (**D190a corrected D184.2** — the label no longer repeats the change); document order proves the change precedes the affordance. **The recommendation is printed once** (D179e). | ✅ `_assertApplyAffordanceShowsItsRecommendation()` (issue panel **+ chat render path**) |
| **C-D184-2** | **NO RENDERABLE RECOMMENDATION ⇒ NO BUTTON.** Blank the `rec` → **zero** apply affordances in the panel. | ✅ state proof in-guard + NC |
| **C-D184-3** | **THE RANK IS COMPUTED.** Feasibility limits → lead **ISS-01**; make Alignment the limit → lead **ISS-05**. **Never a fixed index.** | ✅ `_assertRecommendationRankIsComputed()` |
| **C-D184-4** | **DL-103 §7d IS STRONGER, NOT WEAKER.** The recommendation lives **outside every disclosure row** — the cap cannot hide what was never hideable. Manual editing still free. | ✅ `_assertRecommendationNeverHidden()` (strengthened) + 2 must-not-fire controls |
| **C-D185-1** | ⛔⛔⛔ **THE POPOVER IS A READOUT.** Resident prose **12 words** (budget **25**) · **0** resident explanatory sentences · **0** narration frames · **4 ⓘ affordances still carry the doctrine**. | ✅ `_assertConfPopIsAReadout()` |
| **C-D185-2** | **THE LIMITER HAS A VERB:** *"Feasibility — the lowest. **Confirm** it to lift the read."* And **one way out**, computed: **"Confirm Feasibility →"** → opens ISS-01. **(D196a — the verb moved from `Ground`; the guard clause moved with it. A guard that pins a word the doctrine has moved *is* the doctrine — D166 §3.)** | ✅ guard clause (5) |
| **C-D185-3** | **THE RELIABILITY BASIS SURFACES WHAT IS THIN.** Evidence weakest → *"Thinnest: Evidence — Low."* · two tied → *"Thinnest: Coverage · Evidence — Low."* · all level → *"Even across the basis — Moderate."* (**no invented weakest**, D173). The three rows are **on demand**. | ✅ `_assertReliabilityBasisSurfacesTheWeakest()` |
| **C-D185-4** | **NO REGRESSION.** **110 boot assertions × Free × Basic × notes-OFF/ON × first-value NO/YES (8 configurations) · 0 console errors** · **11 NC suites, 219 controls, every one bites** · every must-not-fire control green · **zero prompts raised by the guard suite** (`d182FenceLifted: true`, `_promptReachesTheUser(): false`) · **AA both themes** (`--subtle` 4.67/4.75 · `--primary-light` 6.30/5.39 · `--text` 13.15/15.05 on `--surface-2`) · body children **31**, unchanged. | ✅ jsdom |

| **C-D186-1** | ⛔⛔⛔ **"HOLDING IT" APPEARS NOWHERE** — not in the rendered copy, not in a tooltip, **not in the TOUR registry**, not in the prototype notes, and not in any render path. The Progress panel's load-bearing line reads **"Your read leans on N inferences — the inferred claims above plus inferred assumptions, relationships and metrics"** (DL-111 + erratum — no `YOUR READ RESTS ON` label). The CAF limiter reads **"Feasibility — the lowest. Ground it to lift the read."** **"Blocker" appears nowhere.** | ✅ `_assertNoHoldingItAnywhere()` (DOM · attributes · `TOUR` · `PN_SLOTS` · render paths) |
| **C-D187-1** | ⛔⛔⛔ **GREEN ONLY WHERE THE USER EARNED IT — AND NEVER RED.** Declared in `COUNT_VALENCE`, **computed**, never typed into markup. User-driven: **`resolved` · `answered` · `vel_you`**. Not user-driven (and each **states why**): `issues` · `critical` · `questions` · `inferred` · `loadbearing` · **`grounded`** · `vel_oslo`. **There is no `red` field to set.** *Measured:* user resolves an issue → **only `resolved ↑1` is green**; OSLO runs a deeper read → **zero greens, zero reds**, and `issues ↑2 · critical ↑1 · questions ↑1 · inferences ↑` all stay neutral. | ✅ `_assertTrendColourIsEarnedOnly()` (7 clauses, 2 state proofs) |
| **C-D187-2** | **THE GREEN IS NOT THE SEVERITY TOKEN.** `--earned` = **#4FC3A1** (dark) / **#0A6E52** (light); `--success` = #4D8B6B / #3E7357. **Chroma distance 77.8 / 52.5.** **AA both themes:** dark **7.61:1** on `--surface`, **6.65:1** on `--surface-2`; light **6.24:1** on `--surface`, **5.67:1** on `--surface-2`. The guard grades the **value**, not the name. | ✅ guard clause (3) + NC |
| **C-D187-3** | **DIRECTION IS STILL THE DATA'S** (D180). Rise → `↑`, fall → `↓`, no movement → **no arrow**. The valence colours; it never points. | ✅ guard clause (4) |
| **C-D188-1** | **A STAT CELL IS `NUMBER · LABEL`.** *Measured:* **4 · Unconfirmed dependencies** (2 w) · **5 · Unowned parties** (2 w) · **3 · Untraceable numbers** (2 w) · **0 · you grounded** (2 w) · **37 · OSLO inferred** (2 w). Budget **≤ 3 words**. **Every one carries an ⓘ**, and the vivid consequence lives there — *one tap away, not resident*. | ✅ `_assertStatLabelsAreLabels()` |
| **C-D189-1** | **"A direction, not a target" IS DELETED.** The remaining captions inform (*"One mark per claim OSLO extracted"* · *"The ones your read rests on come first"*); **none pre-empts a misreading.** The rule is enforced in the code and explained behind the ⓘ. | ✅ `_assertNoPreemptiveCaptions()` (grades the **shape**) |
| **C-D189-2** | **NO FIRST PERSON OUTSIDE CHAT — AND THE GUARD CAN NOW SEE THE ISSUE PANEL.** `#issuePanel`/`#ipBody` matched **nothing**; the panel was never graded. Fixed, plus **every surface in the list must resolve or the guard FAILS**, plus the panel is **opened under the probe fence** and graded with an issue in it. The **user's own controls** still say *"my"*, by declaration. **The readout still speaks in the PM's first person (D152); chat still says "I" (D165).** | ✅ `_assertOsloSaysIOnlyInChat()` (clause 0 + clause e) |
| **C-D189-3** | **NO REGRESSION.** **113 boot assertions × Free × Basic × notes-OFF/ON × first-value NO/YES · 0 console errors, every configuration** · **12 NC suites, 245 controls, every one bites** · every must-not-fire control green · **zero prompts raised by the guard suite** (`d182FenceLifted: true` · `_PROBE.depth: 0` · `_promptReachesTheUser(): false` · `_upDeferred: 0`) · `node --check` PASS · inert jsdom (no `runScripts`) **body children 31, unchanged** · Slices 1–9 guards all green. | ✅ jsdom |
...
|---|---|---|
| **C-D190-1** | ⛔⛔ **THE AFFORDANCE IS SHORT AND CONSTANT.** *Measured across all six issues with a renderable recommendation:* **one** label — **"Apply this fix"** — and six different fixes above it. The chat's apply affordance reads **"Apply this fix →"** from the **same** reader (`_applyLabel()`). | ✅ `_assertApplyAffordanceShowsItsRecommendation()` (clause 3 = the constant · clause 3b = **constancy proven by perturbing the subject**) |
| **C-D190-2** | ⛔⛔⛔ **AND THE CONSENT CLAUSE STILL BITES.** **Blank the recommendation → the guard goes RED** (verified). The fix is still **visible, in the same block, out of every drawer, ABOVE the button** (document order), and **no renderable recommendation ⇒ NO BUTTON**. **The doctrine did not move — only the guard's label clause did (D166 §3).** | ✅ NC `aButtonWithNoSubjectSurvives_bites` · `theFixGoesBackIntoAClosedDrawer_bites` · `theButtonClimbsAboveTheChange_bites` |
| **C-D190-3** | **"PATH" IS GONE, IN THIS SENSE.** **Zero** occurrences in rendered copy — DOM text, `title`/`data-tip`/`aria-label` attributes, the chat render paths, the review kinds, the **History** entries and the **tour**. *"Other options" · "Selected option" · "Resolution option selected" · "You commit to an option" · "You'd take a different option".* **Internals keep `paths[]`/`_selpath`** (the D183e split). | ✅ DOM + attribute + render-path sweep |
| **C-D190-4** | ⛔⛔⛔ **THE OPTIONS OPEN IN EXACTLY ONE PLACE.** *Measured:* **1 opener** · each option rendered in **exactly 1** leaf site · `#ipAlts` **inside** `#ipRecBlock`, **after** the recommendation text, **not** inside `.ip-rows`/`.ip-row`. The disclosure row under Evidence is **gone** — the panel's rows are **Evidence · Clarification · Comments**(· Reviews). | ✅ `_assertOptionsHaveOneHome()` + **NC** `aSecondOpenerForTheSameSet_bites` · `theOptionsAreRenderedTwice_bites` · `theOptionsGoBackUnderEvidence_bites` |
| **C-D190-5** | ⛔⛔⛔ **THE CAP STILL NEVER HIDES THE RECOMMENDATION, AND THE FREE DOOR SURVIVED THE MOVE.** *Measured at a simulated cap:* recommendation **visible**, **not** in a drawer · `#ipAlts` **forced open** · *"✎ Write my own fix in Resources →"* **on screen** · the user **cannot collapse it**. **Metering understanding stays PROHIBITED (D126/D128).** | ✅ must-not-fire `mustNotFire_atTheCapTheFixIsVisibleAndTheFreeDoorIsOpen` + `_assertRecommendationNeverHidden()` |
| **C-D190-6** | **AA, BOTH THEMES — including the contrast this move surfaced.** New `--success-fg` (dark **#6FB894** / light **#3E7357**): **5.61:1** and **5.58:1** dark, **4.63:1** and **4.66:1** light (was **3.74 / 3.72 dark — below AA, pre-existing**). Every other foreground on the moved block passes: `--muted` 7.67/7.29 · `--subtle` 4.67/4.75 · `--primary-light` 6.30/5.39 on `--surface-2`. **`--success` untouched** (D187's distance guard unaffected). | ✅ contrast harness |
| **C-D190-7** | **NO REGRESSION.** **114 boot assertions × Free × Basic × notes-OFF/ON × first-value NO/YES (6 configurations) · 0 console errors, every configuration** · **12 NC suites, 251 controls, every one bites** · every must-not-fire control green · **zero prompts raised by the guard suite** (`_PROBE.depth: 0` · `_promptReachesTheUser(): false` · `_upDeferred: 0`) · `node --check` PASS · inert jsdom (no `runScripts`) **body children 31, unchanged** · Slices 1–9 + the rest of Slice 10 green. | ✅ jsdom |
...
| ID | Criterion | Verified |
|---|---|---|
| **C-D191-1** | ⛔⛔⛔ **THE DECISION COMES BACK.** *Measured, end to end, on the live product:* apply a fix → **withdraw it** → the document is **restored to its pre-fix version** (v3 → **v2**, pre-fix was v2) · the attestation is **gone** (`attested` → **`derived`**) · **Reliability is back to its prior value** (`Moderate` → **`Low`**) · the issue is back to **Open** · the selection is back to **NO selection** · **History has MORE rows, not fewer** (1 → 4 → **7**) · the assisted apply is **refunded** (1 → **0**, and **recorded**). | ✅ behavioural harness (jsdom, real paths, real timers) |
| **C-D191-2** | ⛔⛔⛔ **THE READ DID NOT ROLL BACK — IT RE-RAN.** *Measured:* immediately after the hand-path the read is **byte-identical** (`Moderate/Low/38/62` — last-good, **D098g**). Then the **analysis update** lands and the read moves to `Moderate/Very Low/30/58`, with **1 new run event** and **1 new trend point** (*"the fix was withdrawn — venue wi-fi capacity is unconfirmed is open again"*). **A NEW read, not the old one restored.** | ✅ `_assertNoHandPathMovesTheRead()` (record shape · source scan · state proof) + behavioural harness |
| **C-D191-3** | ⛔⛔⛔ **NO TRANSITION INTO `addressed`/`attested` EXISTS WITHOUT AN INVERSE.** The table is **enumerated** (`_ISSUE_TRANSITIONS`), each inverse is **driven** on the live product (the **round trip**: Open → Addressed → **Open**), and the whole product is **swept** for unregistered writers. **A new one-way transition FAILS THE BUILD.** | ✅ `_assertEveryDecisionTransitionHasAnInverse()` · NC `theTransitionLosesItsInverse_bites` · `aNewOneWayTransitionIsAdded_bites` · `aNewOneWayAttestationIsAdded_bites` |
| **C-D191-4** | ⛔⛔⛔ **THE EDIT AND THE ATTESTATION MOVE AS ONE UNIT.** One function may drop an attestation (`_withdrawUnit`), and it **cannot return without having restored the document**. **Withdraw one without the other → RED**, proven in **both directions**. | ✅ `_assertWithdrawMovesEditAndAttestationTogether()` · NC `theEditGoesButTheAttestationStands_bites` · `theAttestationGoesButTheEditStays_bites` |
| **C-D191-5** | ⛔⛔⛔ **HISTORY NEVER SHRINKS.** The withdrawal is a **new event**; the origin event survives **byte-identical**. **Delete it → RED. Relabel it → RED.** Nothing in the product may cut the record (swept). | ✅ `_assertWithdrawalNeverShrinksHistory()` · NC ×2 |
| **C-D191-6** | ~~**WITHDRAW IS ABSENT ON A RESOLVED ISSUE.**~~ ⛔⛔⛔ **REVERSED BY D192a — THE GUARD I WAS ORDERED TO WRITE ENCODED THE DOCTRINAL ERROR AND WOULD HAVE HELD THE DEFECT IN PLACE.** *(It passed. It was measuring the wrong thing.)* **See C-D192-1.** | — |
| **C-D191-7** | ⛔ **THE AFFORDANCE IS NAMED FOR WHAT IT DOES, AND NEVER SAYS "UNDO."** *Measured on the live DOM:* the panel offers **"Withdraw this fix"** / **"Withdraw this answer"** / **"Clear selection"**; the History row offers the same word on the record of the decision itself; **zero occurrences of "undo"** in the issue panel. *"Undo" implies the world returns to how it was — and the read does not.* | ✅ behavioural harness |
| **C-D191-8** | ⛔ **ONE LINE OF CONSENT, BEFORE IT ACTS.** *Measured:* *"This removes the change from Resources and withdraws your confirmation. OSLO will re-read the plan."* — rendered **with the subject on screen** (D184), and **the state is untouched at the consent step** (status still `addressed`, document still v3, basis still `attested`). A withdraw raised **from the History row opens the issue first**, so the subject is always present. | ✅ behavioural harness |
| **C-D191-9** | ⛔ **THE REFUND.** *Measured on the Usage & Limits surface:* **"0 applied · no limit / 1 refunded (withdrawn)"**. The **analysis run is not refunded** — the monthly meter reads **2 used** (the apply's run, and the withdrawal's), because both really happened. *You cannot bill a user for labour you took back; you also cannot un-run an analysis.* | ✅ `_assertWithdrawalRefundsTheAssistedApply()` · NC ×2 |
| **C-D191-10** | **AA, BOTH THEMES.** `.ip-wd .wd-line` **7.92 / 7.39** · `.ip-wd-still` **7.88 / 7.27** *(replaces `.ip-wd-absent`)* · `.hwd` **7.20 / 5.93** · `.meter .mref` **4.67 / 4.75** (dark / light; AA = 4.5). `.hwd:hover` does not recolour — the underline weight carries the feedback. | ✅ contrast harness, both themes |
| **C-D191-11** | **NO REGRESSION.** **123 boot guards × Free × Basic × notes-OFF/ON × first-value NO/YES (6 configurations) · 0 console errors, every configuration** · **13 NC suites, 281 controls, every one bites** · every must-not-fire green · **zero prompts raised by the guard suite** (`_PROBE.depth: 0` · **164 prompts computed, 0 shown** · `_promptReachesTheUser(): false` · `_upDeferred: 0`) · **the nine state-staging guards are graded by the D182 residue guard and leave no byte behind** · `node --check` PASS · inert jsdom **body children 31, unchanged** · Slices 1–9 + the rest of Slice 10 green. | ✅ jsdom |

---

## D192 / D193 — the erratum, and the two questions D191 exposed (owner, 2026-07-13)

> ## **WITHDRAWING A FIX IS NOT HAND-MOVING THE READ.** · ## **OSLO MAY NEVER DELETE THE USER'S OWN WRITING.**

| ID | Criterion | Verified |
|---|---|---|
| **C-D192-1** | ⛔⛔⛔ **THE WITHDRAW SURVIVES RESOLUTION.** *Measured, end to end, with real timers:* apply → **wait past the ~1.9s analysis update** → the issue is **Resolved** → **`_wdAvailable` is TRUE**, the panel offers **"Withdraw this fix"**, the History row offers it, and the panel **states that the word still stands**. *Before D192a the affordance was gone by this point and the attestation was permanent again — the P1, re-entering through §5.* | ✅ `_assertWithdrawSurvivesResolution()` + behavioural harness (real timers, real paths) |
| **C-D192-2** | ⛔⛔⛔ **THE ISSUE RE-OPENS BY ANALYSIS, NOT BY HAND.** *Measured:* immediately after the hand-path the issue is **still `resolved`** and the read is **byte-identical** (`Moderate/Low/38`) — **the user did not move it.** ~2s later the **analysis update** lands: the issue is **Open**, a **lifecycle event** is filed (*"Venue Wi-Fi capacity is unconfirmed — Open again"*), the read re-runs to `Moderate/Very Low/30`, **1 new run event · 1 new trend point.** | ✅ `_assertWithdrawSurvivesResolution()` (*handDidNotMoveTheStatus* · *reopenedByAnalysis*) · NC `theHandPathReopensTheIssueItself_bites` · NC `theAnalysisUpdateNeverReopens_bites` |
| **C-D192-3** | ⛔⛔ **THE LIFECYCLE CHEVRON NO LONGER DRAWS A RATCHET.** *Measured on the live DOM across a full round trip:* `Open[on] ⇄ Addressed ⇄ Resolved` → `Addressed[on]` → `Resolved[on done]` → **withdraw + analysis update** → **`Open[on]` again.** **The track came back.** Zero one-way arrows anywhere in it; **only the state the issue is in is lit.** | ✅ `_assertLifecycleIsNotDrawnAsARatchet()` (glyphs · fill · **the backwards drive**) · NC ×2 |
| **C-D193-1** | ⛔⛔⛔⛔ **THE USER'S EDIT SURVIVES A WITHDRAWAL.** *Measured, end to end:* apply (v2 → **v3**) → **the user writes in the document** through the real editor (**v4**, +1 sentence) → withdraw → **their sentence is still there, byte for byte** · **the stored document did not shrink** (12,299 → **12,299** bytes) · **no restore** (still **v4**) · **the attestation dropped anyway** (`attested` → `derived`, Reliability `Moderate` → `Low`) · **an analysis update ran** · **History grew.** | ✅ `_assertWithdrawalNeverDeletesTheUsersWriting()` (+ the control: an **untouched** document **is** still restored) · NC `theRestoreDestroysTheUsersEdits_bites` · NC `theAttestationSurvivesAnEditedDocument_bites` |
| **C-D193-2** | ⛔⛔ **AND OSLO SAYS SO PLAINLY.** *Measured, in the consent line before it acts:* *"This withdraws your confirmation. **Your edits since are kept** — OSLO's change is still in Resources, and **you can remove it yourself if you want it gone.** OSLO will re-read the plan."* — and the History event repeats it. **A silent non-restore is a lie of omission.** | ✅ NC `theNonRestoreIsNotStated_bites` |
| **C-D193-3** | ⛔⛔⛔ **THE ATTESTATION IS REFCOUNTED BY DECISION.** *Measured on `Resources`, with the state CONSTRUCTED (a vacuous pass here is a failure):* two fixes attest it (`derived/Low` → `attested/Moderate` → `attested/High`) → **withdraw one ⇒ the document STAYS `attested`** (`High`, one decision still standing), and the History event says why → **withdraw the last ⇒ it DROPS to `derived`, and Reliability returns to `Low` — its PRE-*FIRST*-attestation value**, not one step back. | ✅ `_assertAttestationIsRefcountedByDecision()` · NC `withdrawingOneOfTwoDropsTheAttestation_bites` · NC `theReliabilityRestoresOneStepBack_bites` |
| **C-D193-4** | ⛔ **THE STANDING PROHIBITION STILL BINDS.** The hand-path writes **no band, no CAF width, no Confidence, and no `resolved` status.** The record it restores from **contains no read** — the mechanism, not a promise. | ✅ `_assertNoHandPathMovesTheRead()` (record shape · source scan · state proof) · NC `theWithdrawWritesTheReadDirectly_bites` · NC `theRecordStartsCarryingABand_bites` |

## D194 (owner, 2026-07-13) — say it once · say it in the ratified vocabulary · do not merge the rows

> **⛔ DL-111 + erratum (2026-07-14).** The panel was rebuilt from the two-`.pg-row` ledger into a **foundation
> bar**: a computed **grounded-facts hero (ATTESTED claims only), shown as a fraction `N of TOTAL`** (WI-R6), a **proportional solid bar** showing **two
> provenance states** — *Confirmed by you* (grounded; cool accent; label only, its count is the hero) and *From
> OSLO* (**inferred**; hatched) — and **load-bearing as its OWN line below the bar, a superset** (*"Your read leans
> on N inferences — the inferred claims above plus inferred assumptions, relationships and metrics · See them →"*).
> The ledger-**structure** criteria below (say-it-once, two `.pg-row` hosts) are retired; **single-source** survives.
> **Three POPULATION guards were added — the old guards passed the erratum's lies because they only proved
> *computed* and *registry-sourced*, neither of which was wrong:** `_assertPgxBarIsComputedFromRealCounts()`
> (**hero == grounded, RED on grounded+inferred**), `_assertPgxTwoProvenanceStates()` (**exactly TWO classes, RED on
> any *derived/supported* third**), `_assertPgxBarStructure()` (**load-bearing is a superset line, RED if drawn
> inside the bar or `+`-joined**). Colour discipline: **red to Critical**, **orange off state**, deltas neutral.

| # | Criterion | Verification |
|---|---|---|
| **C-D194-1** | ⛔ **THE LOAD-BEARING LINE SAYS THE NUMBER AND THE NOUN, ONCE.** (DL-111 + erratum retired the two-`.pg-row` say-it-once structure.) The *leans* line **below the bar** reads **"Your read leans on 12 inferences — the inferred claims above plus inferred assumptions, relationships and metrics · See them →" ↓7** — count, noun, superset gloss, neutral delta and the map link, with **no separate label to echo**. | ✅ `_assertPgxBarStructure()` — the load-bearing line carries its count and noun and is **never inside the bar / `+`-joined**; the neutral delta is proven by **moving the baseline** (`↓7`) |
| **C-D194-2** | ⛔⛔⛔ **THE BAR'S SEGMENTS SPEAK THE RATIFIED EPISTEMIC CLASSES.** *Rendered (erratum):* the grounded segment names **Confirmed by you** (label only — its count is the grounded numerator of the **17-of-28 hero**) and the inferred, hatched segment names **From OSLO 11**. **Exact ratified strings** (D011/D069). | ✅ `_assertEpistemicClassNamesAreSingleSourced()` clause (e) — the bar segments must NAME both classes, read from the registry |
| **C-D194-3** | ⛔⛔⛔ **NO SURFACE PRINTS AN EPISTEMIC-CLASS NAME FROM A LITERAL.** One registry (`EPI_CLASSES`), one reader (`epiClassName()`), one painter. **18 label sites routed** — the document list, the block hover chip, the table cell + row-gutter labels, the Outcome-Confidence chip, the Inference-map key, the issue panel's two chips, the readout scaffold, the reviewer view, and the third-class chips. **8 `[data-epi-class]` elements painted at boot.** | ✅ **Mechanism proof:** rename the classes **in the registry** → repaint → **every label follows** (a label that does not is a literal; a sentinel that lands on <2 labels is a **vacuous** guard and FAILS) · **plus a source scan** of 10 render paths (comments stripped — a comment is not copy) · NC-D194-05…07 |
| **C-D194-4** | ⛔⛔⛔ **THE THIRD CLASS IS REPRESENTABLE.** The bar is **computed over `EPI_CLASSES`**, not two hard-coded slots. Today the third class is **ABSENT** — reviewer evidence attaches to an **ISSUE**, not a `ContextItem` (D115) — and **absent is correct** (D173: a count OSLO cannot compute is not shown; **never a zero**). | ✅ `_assertThirdEpistemicClassIsRepresentable()` — *measured:* give a live `ContextItem` an `EV-AT3-Dana Whitfield` evidence id → **a third segment appears, reading *"Attested by Dana Whitfield 1"*, with its own `data-count-key`, and it displaces neither of the other two.** Every byte restored · NC-D194-08…10 · **must-not-fire:** absent-and-representable |
| **C-D194-5** | ⛔⛔ **THE SOLID BAR AND THE LOAD-BEARING LINE COUNT DIFFERENT POPULATIONS.** (DL-111 + erratum.) The bar's inferred segment counts inferred **claims** (11); the leans line counts inferred items of **every** type (12), so **12 legitimately exceeds 11** — a superset, **never `+`-joined** into the bar. **No denominator, no percentage, no "of", no ratio glyph** on **either row** (the hero's `17 of 28` fraction is a distinct third element above the bar — WI-R6 — not one of these two rows). | ✅ `_assertPgxBarStructure()` (load-bearing never inside the bar / `+`-joined) · `_assertProgressRowsStayDistinct()` · NC-D194-11 (the merge, built) · NC-D194-13 (the counts read as a ratio) |
| **C-D194-6** | ⛔⛔ **THE ORPHANED PHRASING IS GONE.** *"Your evidence:"* / *"OSLO inferred:"* appear **nowhere** in rendered product copy, **nor in the TOUR registry, nor in `PN_SLOTS`** (the tour has twice been caught teaching a dropped concept). The Inference-map key now names the **same two classes** the ledger does. | ✅ jsdom DOM sweep (scripts/styles excluded) + TOUR + `PN_SLOTS` registry scans: **GONE** |
| **C-D194-7** | ⛔ **D187 SURVIVES INTACT.** `grounded`, `inferred` and the new `attested3` are **NOT user-driven** (O-D187-1: OSLO can extract a new claim from a document the user already confirmed; a **reviewer** — not the user — moves the third class). `resolved` / `answered` **are**. **There is no `red` field to set, anywhere.** *(Measured: `↓7` on load-bearing = `pg-d` neutral · `↑2` on issues = `pg-d` neutral · `↑1` on resolved = `pg-d earned` — green.)* | ✅ `_assertTrendColourIsEarnedOnly()` green · must-not-fire `theTrendColourRulesAreIntact` |
| **C-D194-8** | ⛔ **NO REGRESSION.** **128 boot guards green across a 6-config matrix** (first run · signed-in · first-value · Basic · Pro+light · notes-on), **0 console errors**. **14 NC suites / 302 controls — every one bit or held.** **D182: zero prompts reach the user** from the entire harness; the probe blind is down and the fence lifts clean. | ✅ jsdom |
| **C-D194-9** | ⛔ **COLOUR DISCIPLINE + AA, BOTH THEMES (DL-111 + erratum).** The *From OSLO* segment **is the inferred state**, rendered **hatched**; the *Confirmed by you* (grounded) segment carries a **cool-blue accent** (`--cool`) that **echoes the Outcome Confidence ramp's lit band** — a **harmony cue, not a RAG token**. **Severity red is scoped to the Critical count alone** (`.pgx-ws.crit`); **brand orange stays on actions/links (`See them →`, `Timeline →`), never on state**; deltas neutral. **AA holds in both themes.** | ✅ computed contrast · the colour guard reddens on severity red outside `.pgx-ws.crit` |

## D195 (owner, 2026-07-13) — a class that resolves to nothing · plans do not infer · the flag's counts are computed

| # | Criterion | Verification |
|---|---|---|
| **C-D195-1** | ⛔⛔⛔⛔ **THE *USAGE & LIMITS* DIALOG HAS AN OPAQUE PANEL — MEASURED, NOT ASSERTED.** *Rendered:* `<div class="wmodal">`; **computed background `#1B1F24` (dark) / `#FFFFFF` (light), alpha 1.0**; border 1px; radius 16px. **The page no longer bleeds through it.** | ✅ measured through `getComputedStyle` → `var()` chain resolved against the theme's `:root` → parsed to RGBA |
| **C-D195-2** | ⛔⛔⛔ **EVERY DIALOG DOES. ALL 25 PANELS, BOTH THEMES, ALPHA 1.** 6 × `.wmodal` · 3 × `.modal` · `.upx` · `.issuepanel` · `.palette` · `.scopedpanel` · the History-seam dialog · the Notifications drawer · `#confpop` · `#rptPop` · `#linkPop` · `#tourtip` · `#projmenu` · `#acctmenu` · `#rtTurnMenu` · `#rtToolbar` · `#findBar` · `#annoPop` *(created lazily — the guard **instantiates the real element**, measures it, and puts it back)*. | ✅ `_assertEveryDialogHasAnOpaquePanel()` — **it grades the COMPUTED STYLE, never the class name** *(the whole point: the class name looked right)* |
| **C-D195-3** | ⛔⛔⛔ **THE GUARD CANNOT PASS FOR FREE.** An **empty enumeration** ⇒ RED · a **registry selector that matches nothing** ⇒ RED · a **blinded colour resolver** ⇒ RED, *without grading a single panel*. The apparatus proves itself on four probes (opaque · `rgba(…,0)` · `transparent` · **unpainted**) **before** it grades anything. | ✅ NC-D195-07 · NC-D195-08 · NC-D195-10 |
| **C-D195-4** | ⛔⛔⛔ **A NEW DIALOG CANNOT SHIP UNGRADED.** **(i)** every `*Scrim` overlay that holds anything must yield a **registered** panel; **(ii)** every `role="dialog" \| "alertdialog" \| "menu"` element must **BE** a registered panel or **CONTAIN** one. | ✅ NC-D195-09 — a brand-new scrim + dialog is injected and the guard finds it |
| **C-D195-5** | ⛔⛔⛔ **ZERO DANGLING CLASS NAMES IN THE WHOLE DOCUMENT.** Swept across **three** surfaces: the **live DOM** (638 classes) · the **render paths** (`class="…"` literals in every product function) · the **writes** (`classList.add/toggle`). **8 found. 8 fixed. 0 remain.** *A name resolves if the cascade paints it **or** the product reads it; **a write is not a reader.*** | ✅ `_assertEveryClassNameResolves()` — **0 dangling** · NC-D195-11/12/13 |
| **C-D195-6** | ⛔⛔⛔ **THE SECOND `.wm` — FOUND AND FIXED.** The Readout's **Signed** input carried `class="in"`, **defined nowhere**, so the field the memo goes out under rendered as a **raw browser input** inside a themed popover — **and only when the popover opened**, which is why no DOM scan had ever seen it. Now `.wm-in`. | ✅ measured · **AA 13.15:1 dark / 15.05:1 light** |
| **C-D195-7** | ⛔⛔ **THE SIX HARMLESS DEAD NAMES ARE REMOVED, NOT STYLED.** `.attach-hint` · `.hart` · `.pg-star` · `.hli` · `.pn-on` · `.superseded`. **Nothing painted them and nothing read them.** **No rule was invented to make a dead name resolve** — that would be inventing product design to satisfy a guard. | ✅ zero visual change (each element's appearance was already carried by a sibling class or its own inline styles) |
| **C-D195-8** | ⛔⛔ **A DATA VALUE NO LONGER LIVES IN THE CLASS NAMESPACE.** `class="wl-sig <key>"` → `class="wl-sig" data-sig="<key>"`; `.wl-sig.review` → `.wl-sig[data-sig="review"]`. **`referral` was a class that resolved to nothing, and every future key would have been another one.** | ✅ identical rendering · AA unchanged (4.67:1 / 4.75:1) |
| **C-D195-9** | ⛔⛔⛔ **OSLO IS THE INFERRING ACTOR.** *Rendered:* **"Where OSLO inferred"**. **No surface — on screen, in a render path, or in the TOUR registry — makes the plan / document / artifact / report the SUBJECT of an inference verb.** *"Inferred **FROM** the plan"* stays legal: **that is a SOURCE, and it is the mechanism.** Same actor as the ratified **"From OSLO"** (D194c). | ✅ `_assertOsloIsTheInferringActor()` — **the RULE, not the string** · NC-D195-14…17 (including *the owner's own words, adopted as written*, and *the actor deleted altogether*) |
| **C-D195-10** | ⛔⛔⛔ **THE FALSE-CONFIDENCE FLAG'S COUNTS FOLLOW THE STATE.** *Rendered:* *"**Scope** reads strong — but **4 of 7** items read as inference, most from OSLO. Worth verifying first."* → **three inferences added to Scope** → *"**7 of 10**"*. **A literal cannot do that.** Both numbers read off `_ciArtStats()` — **one home** (D179e). | ✅ `_assertFalseConfidenceFlagCountsAreComputed()` — **STATE PROOF**, not a copy scan · NC-D195-18 (both typed) · NC-D195-19 (**half-computed is not computed**) · **does not duplicate NC-D109-10**, which proves the flag's *subject* |
| **C-D195-11** | ⛔ **NO REGRESSION.** **132 boot guards green across the full 8-config matrix** (Free/Basic × notes OFF/ON × first-value NO/YES), **0 console errors**, **31 body children** (jsdom, no `runScripts`). **15 NC suites / 327 controls — every one bit or held.** **D182: zero prompts reach the user** from the entire harness; probe depth 0, no `data-probe`, 0 deferred prompts, body children **31 → 31**. | ✅ jsdom |
| **C-D195-12** | ⛔ **AA, BOTH THEMES, ON EVERY TOUCHED SURFACE.** `.wm-in` **13.15 / 15.05** · `.wmodal` body **15.05 / 16.56** · `.wm-sub` **8.77 / 8.03** · `.wl-sig` **4.67 / 4.75** · `.inf-sub` **8.77 / 8.03** · `.inf-flag` **15.05 / 16.56**. **No new colour token was introduced.** | ✅ computed contrast |

## D196 — THE USER CONFIRMS. THE READ IS GROUNDED. (owner, 2026-07-13)

| ID | Criterion | Verification |
|---|---|---|
| **C-D196-1** | ⛔⛔⛔ **EVERY USER-FACING IMPERATIVE SAYS *CONFIRM*.** *Rendered:* `#ov-limit` and `#cpp-limnote` → ***"Feasibility — the lowest. Confirm it to lift the read."*** · `#cpp-out` → ***"Confirm Feasibility →"*** · `#cg-feas-tipb` → ***"…the lowest dimension — confirm it to lift the read."*** | ✅ `_assertConfirmIsTheVerbAndGroundedIsTheState()` clause (c) — **measured on the rendered elements, not asserted** |
| **C-D196-2** | ⛔⛔⛔ **EVERY RELIABILITY STATE STILL SAYS *GROUNDED*.** `_GROUNDING_WORDS` = *barely · thinly · partly · largely · well **grounded***; `#cpp-grdword` · `#ov-rel` · `#cp-grd` all render *"…**grounded**"*, **computed from `CONTEXT_ITEMS`, never typed** (D183c). | ✅ clauses (a) + (b) |
| **C-D196-3** | ⛔⛔⛔ **"CONFIRMED BY YOU" SURVIVED THE SWEEP AS AN EPISTEMIC CLASS.** `epiClassName('you')` = **"Confirmed by you"**, and it **renders on the foundation bar's grounded (cool-accent) segment** (DL-111 + erratum; alongside the inferred, hatched *From OSLO 11* segment). **The verb moved to *Confirm* because the class name already said it — a sweep that renamed the class would have destroyed its own justification.** | ✅ clause (e) + `NC-D196-11` |
| **C-D196-4** | ⛔⛔⛔ **NO *"GROUND"* IMPERATIVE ANYWHERE THE WORDS REACH A HUMAN.** Swept: **9 views** of rendered DOM · **every `title`/`aria-label`/`data-tip`/`placeholder`** · the **TOUR registry** (0 hits) · **`PN_SLOTS`** (0 hits, 7 slots) · **8,499 product string literals** across the render paths. | ✅ clause (d) · **0 violations** |
| **C-D196-5** | ⛔⛔ **THE GUARD FOUND A SITE THE GREP DID NOT.** `#cg-feas-tipb` — the hover-only CAF-row tooltip — still said *"ground it to lift the read"*. **The manual sweep missed it; the mechanism did not.** | ✅ **This is D196c's whole point: the split is MECHANICAL, not remembered.** |
| **C-D196-6** | ⛔⛔⛔ **THE GUARD THAT WAS HOLDING THE OLD WORD IN PLACE IS FIXED.** `_assertConfPopIsAReadout()` clause (5) asserted `/\bground\b/`; it now asserts `/\bconfirm\b/`. **A guard that pins a word the doctrine has moved has become the doctrine** (D166 §3). | ✅ **guard defect #34** |
| **C-D196-7** | ⛔⛔ **THE ROLE, NOT THE SUBSTRING — SIX MUST-NOT-FIRE CONTROLS.** *"Confirmed by you"* (class) · *"largely grounded"* (state) · *"you grounded"* (past tense, D187 green) · *"solid ground"* (noun) · *"background"/"groundbreaking"* (word boundaries) · *"Progress is grounding, not clearing"* (objectless gerund) — **all still legal, all still on screen.** | ✅ `_d196NegativeControls()` — **6/6 green** |
| **C-D196-8** | ⛔ **NO REGRESSION.** **132 boot guards green × the full 8-config matrix** (Free/Basic × notes OFF/ON × first-value NO/YES) · **0 console errors** · **16 NC suites, 345 controls, 0 dead** (`_d196NegativeControls()` = **18**) · **D182: 0 prompts reach the user** (`_PROBE.depth 0` · `_promptReachesTheUser() false` · 0 deferred · no `data-probe` · no modal) · body children **31 → 31** · `node --check` **PASS**. | ✅ jsdom |
| **C-D196-9** | ⛔ **AA, BOTH THEMES.** `.cr-limit` **5.34 / 5.34** · `.cr-limit b` **15.05 / 15.05** · `.cpp-limnote` **4.67 / 4.67** · `.caftip` body **4.67 / 4.67** · `.cpp-grd` **4.67 / 4.67**. **D196 added ZERO CSS rules, ZERO colour tokens and ZERO class names** — it is a text change; contrast is unchanged by construction. | ✅ token-resolved contrast, both themes |

## D197 · D198 · D199 — THE NAME · THE MARKER · THE GUARD THAT WAS NEVER BUILT (owner, 2026-07-13)

| ID | Criterion | Verification |
|---|---|---|
| **C-D197-1** | ⛔⛔⛔ **THE TERM IS `LOAD-BEARING` ON THE SURFACES THAT NAME IT.** Count-registry name = **"Load-bearing inferences"**; the assumption marker's `title` and its accessible name = **"Load-bearing"**. **No surface types it** — every one reads `TERMS.loadbearing`. *(DL-111 + erratum: the Progress panel carries no `LOAD-BEARING` label — its load-bearing line is the fixed copy "Your read leans on N inferences — the inferred claims above plus inferred assumptions, relationships and metrics".)* | ✅ `_assertLoadBearingIsTheOneName()` — **MECHANISM PROOF:** the registry is swapped for a **sentinel**, the surfaces are repainted, and **every one must show the sentinel** |
| **C-D197-2** | ⛔⛔⛔ **THE DEAD LABEL IS OFF EVERY SURFACE THE WORDS CAN REACH.** *"YOUR READ RESTS ON"* / *"YOUR READ RESTS ON THIS"* / *"Inferences your read rests on"* — swept from the **DOM**, every **`title`/`aria-label`/`data-tip`/`placeholder`**, the **TOUR**, **`PN_SLOTS`** and the **render paths**. | ✅ **0 hits** · the scanner is **not its own subject** (`DEAD_LB_LABELS` and the guard are excluded) |
| **C-D197-3** | ⛔⛔ ***"RESTS ON" SURVIVES AS PROSE.*** *"The ones your read rests on come first"* · *"What this rests on"* · *"the read rests on the documents you provided"*. **The rule fires on the LABEL FORM only.** | ✅ **3 must-not-fire controls** + a guard clause that **FAILS if the prose disappears** (the over-sweep) |
| **C-D197-4** | ⛔ **THE LOAD-BEARING LINE SAYS IT ONCE** (D194a/D179e). (DL-111 + erratum.) The *leans* line below the bar reads **"Your read leans on 12 inferences — the inferred claims above plus inferred assumptions, relationships and metrics · See them →" ↓7** — the count, the noun and the map link, **no separate label to double**. | ✅ the line carries the number and noun exactly once · **the bar and the leans line stay distinct** (D194d) |
| **C-D198-1** | ⛔⛔⛔ **A MARKED ROW AND AN UNMARKED ROW HAVE IDENTICAL TEXT GEOMETRY — MEASURED.** **(1) in-flow child signature:** `.inf-item` → `span.tx \| span.inf-src` on both · `.inf-row` → `span.inf-nm \| span.inf-pips \| span.inf-ct` on both. **(2) computed box model:** `.inf-item` → `20px \| 2px \| 0 \| 0 \| flex \| 11px` on both · `.inf-row` → `20px \| 8px \| 0 \| 0 \| flex \| 12px` on both. **(3) real pixels:** content start-x/end-x within 1px — live in the guard; **in jsdom every rect is zero, so the clause SAYS SO and does not pass on its own.** | ✅ `_assertMarkedRowsKeepTheirGeometry()` — **measured, not eyeballed** |
| **C-D198-2** | ⛔⛔⛔ **THE MARKER CANNOT ENTER THE FLOW.** `.mk-on::before` and `.mk-sr` are **`position:absolute`** (read from the **merged** cascade — later rules win), and **`.mk-on` declares NO flow property at all.** **The gutter is reserved by the ROW class, for every row.** | ✅ cascade + DOM + box model · `NC-D198-01/03/04` all **RED** |
| **C-D198-3** | ⛔ **NOT COLOUR ALONE, AND NOT SEVERITY COLOUR.** The discriminator is the **existence of a box** (`content` ≠ `none`, `width` > 0). `--cool` on the card: **6.13 : 1 (dark) / 6.28 : 1 (light)** — WCAG 1.4.11 needs **3:1**. No red/amber/green token anywhere in the rule (D003). | ✅ token-resolved contrast, both themes · `NC-D198-05/06` **RED** |
| **C-D198-4** | ⛔⛔⛔ **`VERIFY` IS OUT OF THE NUMBER COLUMN — AND STILL KEYBOARD-REACHABLE.** **0** actionable elements inside the stat rows; **7/7** rows are `role="button"` + `tabindex="0"` + Enter/Space → `openArtifact()`; the `.inf-flag` callout keeps the **named** *"Open Resources →"*. **Two keyboard paths, zero resident buttons.** | ✅ `_assertVerifyIsNotAResidentButton()` · `NC-D198-09/10` **RED** |
| **C-D199-1** | ⛔⛔⛔ **EVERY SURFACE THAT NAMES THE CONCEPT SAYS "OUTCOME CONFIDENCE."** **8 label sites, all resolving:** pill · **hero** · popover · ramp (aria) · **trend ⓘ** · **trend chart (aria)** · whybox · export. **The trend and the export package are DORMANT at boot — the guard PAINTS them, reads them, and restores every byte.** | ✅ `_assertTheConceptIsCalledOutcomeConfidence()` · `NC-D199-01…06` — **revert ANY ONE → RED** |
| **C-D199-2** | ⛔⛔⛔ **GRADE THE ROLE, NOT THE SUBSTRING.** The bare word may never be **(L1)** a whole label · **(L2)** followed by a band or a number · **(L3)** compounded into the concept's name (*trend/band/pill/score*) · **(L4)** the subject of a state verb. ⚠️ ***"is"* is a hit ONLY when a band follows it** — *"Confidence is understanding maturity"* **is the doctrine.** | ✅ **3 must-not-fire controls green** (the doctrine sentence · *"false confidence"* · *"my confidence"*) |
| **C-D199-3** | ⛔⛔ **A SELECTOR MATCHING NOTHING IS A FAILURE.** Hide the trend host → the guard goes **BLIND and RED**, never quietly green. *That is the exact shape of the defect D199 is about.* | ✅ `NC-D199-11` |
| **C-D199-4** | ⛔⛔ **THE SECOND MISS, FOUND BY THE SWEEP:** `PN_SLOTS.pnPayoff` still taught the **superseded D173d** (*"the 0–100 index is DEMOTED, NOT DELETED"*). `_assertNoZeroToHundredIndexAnywhere()` read the DOM, the render paths, the snapshot, the cascade and the TOUR — **and not `PN_SLOTS`.** | ✅ note rewritten · **the guard now reads `PN_SLOTS`** · the notes layer no longer prints the index in any framing |
| **C-D199-5** | ⛔ **NO REGRESSION.** **136 boot guards green × the full 8-config matrix** (Free/Basic × notes OFF/ON × first-value NO/YES) · **0 console errors** · **19 NC suites, 389 controls, 0 dead** (345 + **12** D197 + **16** D198 + **16** D199) · **D182: 0 prompts reach the user** (`_PROBE.depth 0` · `_promptReachesTheUser() false` · 0 deferred · no `data-probe` · 0 modals) · body children **31 → 31** · **9 views swept, 0 violations** · `node --check` **PASS**. | ✅ jsdom |
