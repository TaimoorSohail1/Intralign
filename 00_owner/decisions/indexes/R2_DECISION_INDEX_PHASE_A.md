...
| DL-200–205 | The six Resolve-First rulings = **DR-1…DR-6** (canonical R2 hierarchy: outcome-first, enforce-via-commitment-gate, integrity indicator, phased resolution, activation=2nd act) | **DRAFTS for ratification** (some DR-4/5/6 already applied in-process) | ⚠️ **owner ratify** — ⚠️⚠️ **STATUS COLUMN SUPERSEDED; MARKER WITHDRAWN AND THEN RESTORED, both on 2026-09-02.** The marker was withdrawn on the reading that this row's status column says *"DRAFTS for ratification"*. **That column is dated 2026-08-09 and was superseded on 2026-08-13**, when the compound `DL-200-205_R2_RESOLVE_FIRST_DECISIONS` was split into standalone records on the owner's instruction — each carrying **`Status: Draft → Ratified (Idris)`**, dated 2026-08-04, wording unchanged. **The withdrawal read this column instead of the records, and was wrong.** Restored 2026-09-02. The records live at `release-2/canon/decisions/` and have never reached `main`; that is **graduation debt** (`UNRECORDED_DECISIONS` §A), not an absence of ratification. ★ **A status column in a dated worksheet is a snapshot, not a status — the record is the subject.** [R]|
