# DL-102 — Controlled Release & Tiering-in-Alpha (reconciles CHG-061; amends DL-048 scope, extends DL-049/DL-055/DL-062)

- **Date:** 2026-07-11 · **Status:** Ratified · **Decided by:** Idris (Founder Console)
- **Class:** A
...
- **VOIDS DL-101 — a duplicate of this decision that reached `main` outside the serializer.** DL-101 (`records/DL-101-controlled-release-and-tiering-in-alpha.md`, merged via PR #137) carries the **same decision content** as this record. It was **hand-authored and hand-numbered by an AI contributor from a stale local clone**, in breach of **DL-065 §5** — *"One serializer. The Founder Console is the sole path that authors, numbers, and releases decisions to `main`. No parallel stream merges canon."* It was also **incomplete**: it never received a Changelog Entry, the final step of the Framework 001 lifecycle. **This record (DL-102) is the decision of record**: numbered server-side off current `main` by the `dl-land` workflow (DL-067), with changelog entry **CHG-136**. DL-101 is set to **Status: Void** and reduced to a pointer here — **retained, not deleted**, so the breach remains inspectable.
  *Note:* the mis-numbering DL-101 suffered (drafted DL-099 → renumbered DL-100 → renumbered DL-101 as decisions landed underneath it) is **precisely the failure `dl-land` exists to prevent**, per its own header: *"Numbering happens here, off current `main`, so a stale local clone can never mis-number."*
