...
- **Feedback (commodity · non-canonical · realizes `10_product/experience/IN_PRODUCT_FEEDBACK_CAPTURE_SPECIFICATION_V1.md` §3):** `feedback_submitted` `[C]` (props `feedback_type`=bug/feature/ux/praise, `product_area`=onboarding/mri/findings/recommendations/ingestion/chat/billing, `source`=in_app/survey/support), `survey_shown` `[C]`, `survey_answered` `[C][D]` (props `survey_type`=csat/ces/nps, `trigger`, `score`), `survey_dismissed` `[C]`. *(snake_case realizations of the spec's `Feedback Submitted` / `Survey Shown·Answered·Dismissed`.)*
  - **Boundary (binding — feedback ≠ acceptance):** these **never carry `responsibility`**, never link to or alter an assessment (**OB-5**: only reanalysis changes assessment) — commodity plumbing (DL-043 C/E/F; DL-064 crosswalk), strictly distinct from Wave-U/DL-055 acceptance of a Finding/Recommendation. Internal/test accounts are **excluded from survey sampling** (mirrors the DL-048 internal-entitlement exclusion).
