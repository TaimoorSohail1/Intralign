...
Enforces that the **built app matches the ratified prototype** (`product-design/oslo_r1_experience_mockup_v4.html` — the reference of record). Pairs pixel-diff baselines with the objective `ACCEPTANCE_CRITERIA.md` so "built exactly as designed" is verified at the gate, not trusted.
