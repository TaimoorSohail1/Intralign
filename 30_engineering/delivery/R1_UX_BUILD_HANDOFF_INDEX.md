...
> 2. **The prototype + all 9 visual baselines were updated (PR #116).** Pull latest `main` and build to the **current** `product-design/oslo_r1_experience_mockup_v4.html` — not any earlier read.
...
1. **The prototype is the spec.** `product-design/oslo_r1_experience_mockup_v4.html` is the **reference of record** — build the UX to match it exactly (layout, states, interactions, copy). The experience specs describe intent; where a detail is only visible in the prototype, the prototype wins.
...
- Prototype (reference of record): `product-design/oslo_r1_experience_mockup_v4.html` (+ `OSLO_R1_UX_PROTOTYPE_NOTES_AND_GAP_AUDIT.md`).
