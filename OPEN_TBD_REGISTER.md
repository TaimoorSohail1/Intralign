...
| E4 | **Visual branding / design-token values** (palette, type, spacing, logo, microcopy) | **DO NOT INVENT (escalate).** Token *contract* defined (`RELEASE_1_VISUAL_DESIGN_AND_BRANDING_SPECIFICATION_V1`); **values are designer/owner-TBD.** R1 builds to interim neutral tokens (seed from `intralign.ai`); designer delivery = a single token-swap. The token-adherence lint forbids hardcoded brand. | Visual Spec §1, §4; CHG-067 |
