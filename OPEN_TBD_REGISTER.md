...
| A6 | **Fast/Deep AI call + token budgets** (overall, per-run) | TBD | Perf/NFR §12 |
...
| E3 | Paid-tier limits / relaxed quotas | TBD | Perf/NFR §12; MON capabilities |

## What is NOT on this register (already decided — do not re-open)

The **architecture and epistemic model are settled** (DL-043), the **engineering-enablement layer** is ratified (DL-044), **Fast/Deep + the <60s ceiling** are contracted (DL-046), and the **cognitive contract-coverage gaps** are closed (DL-047). The **< 60-second Time-to-First-MRI** is the **one ratified numeric target.** These are authoritative — build to them; they are not TBD.
